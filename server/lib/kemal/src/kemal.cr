...
module Kemal
  ...
  def self.run(port = nil)
    self.run port do
      ...
      nh = host.gsub("\n", "")
      log "[COMPUTER] Ficha online at #{config.scheme}://#{config.host_binding}:#{config.port}"
      log "[LOCAL NETWORK] Ficha online at #{config.scheme}://#{nh.to_s}:#{config.port}"
    end
  end
  ...
  def self.run(port = nil, &block)
    ...
    unless config.env == "test"
      Signal::INT.trap do
        log "Ficha is Dying!" if config.shutdown_message
        Kemal.stop
        log "Ficha is die"
        exit
      end
  ...
  def self.stop
    if config.running
      ...
      raise "Ficha is already stopped."
    end
  end
end
